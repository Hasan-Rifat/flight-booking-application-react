...
const PreviewData = () => {
  ...
  if (store.length === 0) {
    content = (
      <div className="text-center mx-auto w-full p-4">no data available</div>
    );
  }

  if (store.length > 0) {
    content = store.map(
      ({ id, destinationFrom, destinationTo, journeyDate, guests, Class }) => (
        <tr key={id} className="lws-bookedTable text-black">
          ...
          <td className="px-6 py-4 text-center">
            <span className="lws-bookedClass"> {Class} </span>
          </td>
          <td className="px-6 py-4 text-center">
            <div className="flex justify-center gap-4">
              <button onClick={() => handlerDelete(id)} className="lws-remove">
                <svg
